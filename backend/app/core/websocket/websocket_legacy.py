...
class WebSocketManager:
    ...
    def __init__(self):
        ...
        # Background tasks will be started lazily when first connection is made
    ...
    async def connect(self, websocket: WebSocket, user_id: str, username: str) -> str:
        ...
        # Start background tasks on first connection (lazy initialization)
        ...
        try:
            ...
            # Note: FastAPI/Uvicorn automatically accepts WebSocket connections
            # We should NOT call accept() again as it will cause "already in state CONNECTED" error
            if hasattr(websocket, 'client_state'):
                logger.info(f"WebSocket state: {websocket.client_state.name}")
            
            # Connection is already accepted by FastAPI/Uvicorn
            logger.info(f"WebSocket connection established for connection_id={connection_id}")
